...
class SubtitleTranslator(App):
    '''Р¤СѓРЅРєС†РёРѕРЅР°Р» РїСЂРѕРіСЂР°РјРјС‹.'''

    title = 'SubtitleTranslator'
    icon = 'icon.png'
    ...
    def __init__(self, **kvargs):
        super(SubtitleTranslator, self).__init__(**kvargs)
    ...
    def get_application_config(self):
        return super(SubtitleTranslator, self).get_application_config(
                        '{}/%(appname)s.ini'.format(self.directory))

    def build_config(self, config):
        '''РЎРѕР·РґР°С‘С‚ С„Р°Р№Р» РЅР°СЃС‚СЂРѕРµРє РїСЂРёР»РѕР¶РµРЅРёСЏ subtitletranslator.ini.'''

        config.adddefaultsection('General')
        config.setdefault('General', 'language', 'en')

    def set_value_from_config(self):
        '''РЈСЃС‚Р°РЅР°РІР»РёРІР°РµС‚ Р·РЅР°С‡РµРЅРёСЏ РїРµСЂРµРјРµРЅРЅС‹С… РёР· С„Р°Р№Р»Р° РЅР°СЃС‚СЂРѕРµРє subtitletranslator.ini.'''

        self.config.read(os.path.join(self.directory, 'subtitletranslator.ini'))
    ...
    def show_about(self, *args):
        ...
        self.screen.ids.about.ids.label.text = \
            self.translation._(
                u'[size=20][b]SubtitleTranslator[/b][/size]\n\n'
